# Historical reference: Canton / DAML ledger API

**Production dice.express** uses **Cloudflare D1** for markets and balances. The content below describes a **ledger JSON API** (e.g. Canton) and DAML templates from an earlier architecture. It is not served by the live product.

## Ledger JSON API and DAML templates (reference)
