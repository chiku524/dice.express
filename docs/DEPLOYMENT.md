...
The **deploy** job runs **`npm ci`** at the **repository root** before `wrangler pages deploy`. Root `package.json` supplies **`viem`**, **Solana**, and other libraries that Wrangler bundles with **Pages Functions**; without that install, deploy fails with “Could not resolve …” errors. CI uses **Node.js 22+** because current **wrangler** requires it.
